                ...
                userDatabase.child(uID).addValueEventListener(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        if (!dataSnapshot.hasChild("shifts")) {
                            ...
                            DatabaseReference newShiftRef = userDatabase.child(uID).child("shifts").push();
                            newShiftRef.setValue(newShift);
                            shiftId = newShiftRef.getKey(); // Get the unique key (shiftId) generated by push()
                            Toast.makeText(shiftActivity.this, "Shift successfully created", Toast.LENGTH_SHORT).show();
                            shiftActivity object= new shiftActivity();
                            object.addShift(dateString, startTime, endTime, shiftId);
                        } else {
                            // Check for conflicts
...
//            }
    ...
    private void addShift(String dateString, String startTime, String endTime, String shiftId) {
        View requestView = getLayoutInflater().inflate(R.layout.newshift, null, false);
